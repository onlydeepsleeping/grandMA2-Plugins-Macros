...
-- version: At Color v0.5       - fixed: changed search and application of presets (faster, 3 digit auto complete, no difference between Lee/Rosco (drawback: no R 96 support))
--                              - implemented: added "00" for "Home Preset", input gets corrected to 3 digit string for presets
...
--                              - "00" for "Home" Preset in all preset pool
...
local homePreset = '0.300'      -- define "Home" preset in preset pool
local rangeSUDLcolors = 200     -- define end of SudL color pool, includes default SudL and designer colors
...
local function lookupSudl(inputColor)
    inputColor = string.format('%03d', inputColor)  -- format inputColor as 3 digit string (26 -> 026)
    local i = 1
    local colorDesigner = {}                        -- stores {slot = x, label = "name"}

    -- search names of all color presets from 1 to rangeSUDLcolors
    while i < rangeSUDLcolors do
        local handle = gma.show.getobj.handle("Preset 4." .. i)

        -- check if preset exists (handle) and has a name (label)
        if handle and gma.show.getobj.label(handle) then
            local label = (gma.show.getobj.label(handle))

            -- only add matching colors to table (matched to inputColor)
            if string.find(label, inputColor, 1, true) then
                --  message("Added color " .. label .. " to tabel colorDesigner")
                table.insert(colorDesigner, { slot = i, label = label })
            end
        end
    ...
    -- if no match return
    if #colorDesigner == 0 then
        message("No match in designer colors for: " .. inputColor)
        return
    end

    -- if match(es) (<= 1) apply backwards (probability of hit higher, both for color scroller and for designer colors)
    for index = #colorDesigner, 1, -1 do
        local entry = colorDesigner[index]
        gma.cmd("At Preset 4." .. entry.slot)
        message("Applied Preset 4." .. entry.slot .. ' = "' .. entry.label .. '"')
    end
end
...
local function main()
    ...
    -- 00 == "Home"
    if inputColor == '00' then
        gma.cmd('At Preset ' .. homePreset)
        message('Applied preset "Home"')
        return
    end
...
--                                                                                    no longer needed with new lookupSudl
