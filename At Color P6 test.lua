...
-- version: At Color P6 test    - fixed: error message
--                              - implemented: add support for multiple, to make string input possible, added comments

--          Color Table Cameo P6 Profile            --
--      [Colour name] = MA-Attribute value          --
...
-- Function to perform colour lookup
local function lookupColorP6(colourName)
    local matchingColours = {}
    local colourList = ""  -- Initialize list variable
    for name in pairs(colourTable) do
        if string.find(name:lower(), colourName:lower(), 1, true) then
            table.insert(matchingColours, name)
        end
    end

    if #matchingColours == 0 then
        message("Colour not found")                                               -- add function for pop up? for error ?
        gma.gui.msgbox("Gel not found!", "Colour not found in scroller (P6)")

    elseif #matchingColours == 1 then
        -- get attributeValue from colourTable with gelKey
        local gelKey = matchingColours[1]
        local attributeValue = colourTable[gelKey]

        -- split gelKey apart to remove spaces and format feedback message
        local gelNumber, colourText = gelKey:match("(%d+)%s*|%s*(.+)")

        gma.cmd('Attribute "Scroller" At ' .. attributeValue)
        message("Applied colour: " .. gelNumber .. " - " .. colourText)

    else
        for i, name in ipairs(matchingColours) do
            print(i .. ". " .. name)
            colourList = colourList .. i .. ". " .. name .. " | "
        end

        message("Multiple matches found: " .. #matchingColours)
        gma.gui.confirm("Multiple matches:", colourList)

        local inputChoice = tonumber(gma.textinput("Choose match:"))
        -- check if input is legal choice (isn't nil, 0 or > #matchingColours)
        if inputChoice and inputChoice >= 1 and inputChoice <= #matchingColours then
            -- get attributeValue from colourTable with gelKey
            local gelKey = matchingColours[inputChoice]
            local attributeValue = colourTable[gelKey]
            ...
            gma.gui.msgbox("Error", "Invalid choice")
        end
    end
end

local function main()
    local colourName = gma.textinput("Input gel number", "")
    lookupColorP6(colourName)
end
